...
function removeFromAgenda(title){
  ...
  showActionModal(`Quitar del plan`,`<b>${_s}</b><br><br>Podrás restaurarla desde Sugerencias.`,'Quitar',()=>{
    ...
    const _avr=document.getElementById('ag-view');
    if(_avr) _avr.scrollIntoView({behavior:'smooth',block:'start'});
  });
}
function addSuggestion(title,day,time){
  ...
  // 5. Re-render y scroll al plan
  renderAgenda();
  // Scroll para mostrar el plan actualizado — el usuario puede estar abajo en sugerencias
  const _av=document.getElementById('ag-view');
  if(_av) _av.scrollIntoView({behavior:'smooth',block:'start'});
